...
func enableShareProcessNamespace() (patch []patchOperation) {
	patch = append(patch, patchOperation{
		Op:    "add",
...
func createPatch(pod *corev1.Pod, inj *config.InjectionConfig, annotations map[string]string) ([]byte, error) {
	...
	if inj.ShareProcessNamespace == true && (pod.Spec.ShareProcessNamespace == nil || *pod.Spec.ShareProcessNamespace == false) {
		patch = append(patch, enableShareProcessNamespace()...)
	}
